			...
			if (items) {
				...
				for (var i = 0; i < items.length; i++) {
					...
					if (c_fontname !== fontname) {
						fs.appendFile(srcFolder + '/scss/config/_fonts.scss', '@include font("' + fontname + '", "' + fontname + '", "400", "normal");\r\n', cb);
					}
					c_fontname = fontname;
				}
			}
...
let build = gulp.series(clean, gulp.parallel(addGitIgnore, fontsBuild, imagesBuild, libs, js, others, copyScss, copyHTMLTemplates), webpackBuild, cssBuild, htmlBuild);
let devbuild = gulp.series(clean, gulp.parallel(addGitIgnore, fontsBuild, imagesNoWebpBuild, libs, js, others, copyScss, copyHTMLTemplates), webpackBuild, cssNoWebpBuild, htmlNoWebpBuild);
